...
[source,javascript]
----
window.ExtensionStore.getExtensions(extensionPointId)<1>
----
<1> Extension Store available globally from the window object.



Jenkins relies on reflection to find extensions. This is not possible in Javascript and therefore extensions need to be explicitly registered during the loading of the application. The above code is equivalent to doing the following in Jenkins
:bulb:
Jenkins.getExtensionList(<Extension.class>)

Usage examples will be in following sections as Extension Points and Extension Implementations are defined - window.ExtensionStore will be abbreviated to ExtensionStore.
